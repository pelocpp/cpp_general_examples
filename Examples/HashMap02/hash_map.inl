...
namespace ProCpp {
     ...
     hash_map<Key, T>::hash_map(size_t numBuckets) : mBuckets{ numBuckets } //, mHash{ hash<T>{} }
     {
          if (numBuckets == 0) {
     ...
     //template <typename Key, typename T, typename KeyEqual, typename Hash>
     //hash_map<Key, T, KeyEqual, Hash>& hash_map<Key, T, KeyEqual, Hash>::operator=(hash_map<Key, T, KeyEqual>&& rhs) noexcept
     //{
     //     swap(rhs);
     //     return *this;
     //}
     ...
     template <typename Key, typename T>
     std::pair<typename hash_map<Key, T>::ListType::iterator, size_t>
          hash_map<Key, T>::findElement(const key_type& k)	
     {
     ...
     template <typename Key, typename T>
     void hash_map<Key, T>::insert(const value_type& x)
     {
